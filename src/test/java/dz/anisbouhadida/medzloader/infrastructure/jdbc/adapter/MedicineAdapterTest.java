...
///
/// Verifies that the adapter correctly bridges the [dz.anisbouhadida.medzloader.domain.spi.MedicinePort]
...
class MedicineAdapterTest {
    ...
    class GetMedicineVersionByRegistrationNumberTests {
        ...
        void getMedicineVersionByRegistrationNumber_should_returnVersion_when_medicineFound() {
            ...
            // Act
            int version = medicineAdapter.getMedicineVersionByRegistrationNumber(
                    REGISTRATION_NUMBER, CODE, INN, BRAND_NAME, LABORATORY_HOLDER);

            // Assert
            assertThat(version).isEqualTo(5);
            verify(medicineRepository).findVersionByRegistrationNumber(
                    REGISTRATION_NUMBER, CODE, INN, BRAND_NAME, LABORATORY_HOLDER);
        }

        @Test
        @DisplayName("should return 0 when the repository returns an empty Optional (medicine not found)")
        ...
        void getMedicineVersionByRegistrationNumber_should_returnHighVersion_when_repositoryReturnsIt() {
            // Arrange
            when(medicineRepository.findVersionByRegistrationNumber(
                    REGISTRATION_NUMBER, CODE, INN, BRAND_NAME, LABORATORY_HOLDER))
        ...
        void getMedicineVersionByRegistrationNumber_should_forwardNullArguments_toRepository() {
            ...
            // Act
            int version = medicineAdapter.getMedicineVersionByRegistrationNumber(null, null, null, null, null);
